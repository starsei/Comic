...
function loadlive2d(canvas_id, json_object_or_url, on_load) {
    ...
    console.log("[PioModify] Loading new model")
    ...
    model.once("load", () => {
        ...
        model.on("hit", hitAreas => {
            if (hitAreas.includes("body")) {
                console.log("[PioModify] Touch on body (SDK2)")
                model.motion('tap_body')

            } else if (hitAreas.includes("Body")) {
                console.log("[PioModify] Touch on body (SDK3/4)")
                model.motion("Tap")

            } else if (hitAreas.includes("head") || hitAreas.includes("Head")){
                console.log("[PioModify] Touch on head")
                model.expression()
            }
        })
...
function _pio_initialize_container(){
    ...
    console.log("[PioModify] Initialized container.")
}
